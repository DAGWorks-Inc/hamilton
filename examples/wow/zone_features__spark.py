...
def zone_likelyhoods(zone_counts: ps.DataFrame) -> ps.DataFrame:
    zone_likelyhoods = zone_counts    
    for zone in ["durotar", "darkshore"]:
        zone_likelyhoods = zone_likelyhoods.withColumn(f"{zone}_likelyhood", sf.col(f"{zone}_count") / sf.col("total_count"))
    return zone_likelyhoods
