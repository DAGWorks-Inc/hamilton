...
@post_pipe(step(OneClassSVM_model, nu=value(0.1), kernel=value("rbf"), gamma=value(0.5)))
def model(train_cover_std: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
...
@post_pipe(
    ...
    step(prediction_step, idx_=source("idx"), data_=source("data")),
)
def prediction_train(model: svm.OneClassSVM) -> svm.OneClassSVM:
    return model
...
def prediction_test(model: svm.OneClassSVM) -> svm.OneClassSVM:
    return model
