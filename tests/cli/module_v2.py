...
def average_order_by_customer(amount: pd.Series) -> pd.Series:
    ...
    return (
       amount
         .groupby("customer_id")
         .mean()
         .rename("average_order_by_customer")
    ) + 1
    
def customer_summary_table(
    orders_per_distributor: pd.Series, 
    average_order_by_customer: pd.Series
) -> pd.DataFrame:
    """Our customer summary table definition."""
    return pd.concat(
         [orders_per_distributor, average_order_by_customer], 
          axis=1
    )
